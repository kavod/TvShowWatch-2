...
torrentProvider.TRACKER_CONF.append({'id':'t411','name':'T411','url':'https://api.t411.ai','param':['username','password']})

def connect_t411(self):
	if not self.token: #If no token, let's connect
		req = requests.post(self.provider['url']+"/auth", {'username': self.param['username'], 'password': self.param['password']}, verify=self.SSLverif, timeout=5)
		try:
			result = req.json()
...
def test_t411(self):
	if not self.token:
		...
		req = requests.post(self.provider['url']+"/users/profile/" + self.uid,headers={"Authorization": self.token}, verify=self.SSLverif, timeout=5)
		try:
			result = req.json()
		except JSONDecodeError:
			raise Exception("Invalid response from server: {0}".format(unicode(req)))
...
def search_t411(self, search):
	...
	try:
		...
		result = requests.post(self.provider['url']+"/torrents/search/" + search,headers={"Authorization": self.token}, verify=self.SSLverif, timeout=5)
		result = result.json()
	except JSONDecodeError:
